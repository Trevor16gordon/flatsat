...
| 2026-08-05 | **Ground radio commissioned on the Mac.** `104473b04a06000ffaff320070d5833ac9` = **ground radio** (`pluto-ground`, 192.168.3.1 / `usb:0.3.5` on the Mac). Two-radio link LIVE: artifact staged and telemetry downlinked over 915 MHz GMSK/CCSDS through the 30 dB pads, Mac ↔ Jetson. | P3's first half arrives ahead of schedule; the demo's C1 path now crosses real RF. |
| 2026-08-05 | **Scoped amendment to no-conda: the MAC GROUND MODEM runs from a contained micromamba env** (`~/venvs/gr-ground`, conda-forge gnuradio 3.10.12 + gr-iio, no shell integration). | brew dropped libiio and ships gnuradio without the IIO component; conda-forge has prebuilt arm64 gr-iio. Flight/Jetson env story unchanged (apt + venvs). Version skew vs apt 3.10.1.1 accepted for link traffic; re-pin before BER baselines (B2). |
